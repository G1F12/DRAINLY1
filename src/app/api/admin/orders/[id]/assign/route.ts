...
const bodySchema = z.object({ contractorCompanyId: z.uuid(), reason: z.string().trim().min(10).max(1000) });
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!client) return Response.json({ assignmentPending: true, demo: true });
    ...
    return error instanceof z.ZodError
      ? apiError("BAD_REQUEST", "Invalid assignment", 400, error.flatten())

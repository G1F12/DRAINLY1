...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!client) return Response.json({ overridden: true, demo: true });
    const { error } = await client.rpc("admin_override_authorization", { p_order_id: id, p_reason: body.reason, p_idempotency_key: key });
    if (error) return apiError("FORBIDDEN", error.message, 403); return Response.json({ overridden: true });
  } catch (error) { return error instanceof z.ZodError ? apiError("BAD_REQUEST", "Invalid override", 400, error.flatten()) : apiError("INTERNAL_ERROR", "Override failed", 500); }
}

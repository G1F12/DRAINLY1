...
const schema = z.object({
  minimumContributionMarginCents: z.number().int().min(-100_000).max(1_000_000),
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!client) {
      if (allowDemoFallback()) return Response.json({ quoteId: id, status: "PRICED", demo: true });
    ...
    if (error instanceof z.ZodError) return apiError("BAD_REQUEST", "Invalid economics override", 400);
